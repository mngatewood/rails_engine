...
describe "Items most sold API" do
  it "should return a list of the most sold items" do
    ...
    invoice_item_1 = create(:invoice_item, quantity: 4, unit_price: 2, invoice_id: invoice_1.id, item_id: item_1.id)
    invoice_item_2 = create(:invoice_item, quantity: 2, unit_price: 1, invoice_id: invoice_1.id, item_id: item_2.id)
    invoice_item_3 = create(:invoice_item, quantity: 1, unit_price: 1, invoice_id: invoice_2.id, item_id: item_3.id)
    invoice_item_4 = create(:invoice_item, quantity: 3, unit_price: 100, invoice_id: invoice_2.id, item_id: item_4.id)
    invoice_item_5 = create(:invoice_item, quantity: 1000, unit_price: 1, invoice_id: invoice_3.id, item_id: item_5.id)
    ...
    expect(items.map{|i|i["id"].to_i}).to eq([item_1.id, item_4.id, item_2.id])
  end
end